<div id="form-pegawai">
    ...
    <div class="form-group col-md-6">
        ...
        <select name="cuti_id" id="cuti_id" class="form-control @error('cuti_id') is-invalid @enderror">
            ...
            @foreach ($cutis as $cuti)
                <option {{ $cuti->id == $data_cuti['pivot']->cuti_id ? 'selected' : '' }} value="{{ $cuti->id }}">{{ $cuti->jenis_cuti }}</option>
            @endforeach
        </select>
        @error('cuti_id')
            <div class="invalid-feedback">
    ...
    <div class="form-group col-md-12">
        ...
        <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" cols="30" rows="7">{{ $data_cuti['pivot']->keterangan }}</textarea>
        @error('keterangan')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
    ...
    <div class="form-group col-md-6">
        ...
        <input type="date" name="tanggal_awal" value="{{ old('tanggal_awal') ?? $data_cuti['pivot']->tanggal_awal }}" class="form-control @error('keterangan') is-invalid @enderror">
        @error('tanggal_awal')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
    ...
    <div class="form-group col-md-6">
        ...
        <input type="date" name="tanggal_akhir" value="{{ old('tanggal_akhir') ?? $data_cuti['pivot']->tanggal_akhir }}" class="form-control @error('keterangan') is-invalid @enderror">
        @error('tanggal_akhir')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
